...
namespace DoubleuGames.GameRGD
{
    ...
    [Serializable]
    [NotKeyable]
    public class ControlPlayableAssetEx : PlayableAsset, IPropertyPreview, ITimelineClipAsset
    {
        ...
        [SerializeField] public Vector3 offsetLocation = Vector3.zero;
        [SerializeField] public Vector3 offsetRotation = Vector3.zero;
        [SerializeField] public Vector3 offsetScale = Vector3.one;
        ...
        public override Playable CreatePlayable(PlayableGraph graph, GameObject go)
        {
            ...
            if (sourceObject != null)
            {
                ...
                CreateTransformPlayable(sourceObject, graph, playables);
        ...
        void CreateTransformPlayable(GameObject root, PlayableGraph graph,
            List<Playable> outplayables)
        {
            var transform = TransformControlPlayable.Create(graph, root, offsetLocation, offsetRotation, offsetScale);
            if (transform.IsValid())
                outplayables.Add(transform);
        }
        ...
        internal static void PreviewTransform(IPropertyCollector driver, IEnumerable<Transform> objects)
        {
            // foreach (var transform in objects)
            //     driver.AddFromName<Transform>(transform.gameObject, "position");
        }
