...
/// <summary>
/// NormalizeValue, UpdateVisuals 등을 재정의하기 위해 코드를 모두 복사하여 수정함.
/// </summary>

[AddComponentMenu(UIConst.groupName + "/Mask Slider" + UIConst.suffixUI, 33)]
[ExecuteAlways]
...
/// </remarks>
public class UIMaskSlider : Selectable, IDragHandler, IInitializePotentialDragHandler, ICanvasElement
{
    /// <summary>
    ...
    public RectTransform maskRect { get { return m_MaskRect; } set { if (SetPropertyUtility.SetClass(ref m_MaskRect, value)) { UpdateCachedReferences(); UpdateVisuals(); } } }
    ...
    [SerializeField]
    private float m_MaskValue = 0;

    public float maskValue { get { return m_MaskValue; } set { if (SetPropertyUtility.SetStruct(ref m_MaskValue, value)) { Set(m_Value); UpdateVisuals(); } } }
    ...
    {
        if (m_MaskRect && m_MaskRect != (RectTransform)transform)
        {
            m_MaskImage = m_MaskRect.GetComponent<Image>();
        }
    ...
    private void UpdateVisuals()
    {
        ...
        if (m_FillContainerRect != null)
        {
            ...
            float fillAmount = normalizedValue;
            if (m_MaskImage != null)
            {
                float maskValue = fillAmount;
                if (m_Direction == Direction.LeftToRight || m_Direction == Direction.RightToLeft)
                {
                    maskValue = m_MaskValue / m_MaskRect.rect.width;
                    
                }
                else// if (m_Direction == Direction.TopToBottom || m_Direction == Direction.BottomToTop)
                {
                    maskValue = m_MaskValue / m_MaskRect.rect.height;
                }
                maskValue = Mathf.Clamp01(maskValue);

                if (maskValue > fillAmount)
                {
                    m_MaskImage.fillAmount = fillAmount;
                    fillAmount = maskValue;
                }
                else
                {
                    m_MaskImage.fillAmount = fillAmount;
            ...
            {
                if (reverseValue)
                    anchorMin[(int)axis] = 1 - fillAmount;
                else
                    anchorMax[(int)axis] = fillAmount;
            }
